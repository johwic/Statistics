...
$matches = array($_GET['matchId']);//array_diff(scandir('raw_data/' . $stageId . '/'), array('..', '.'));

$stage = $entityManager->getRepository('Stage')->findOneByWsId($stageId);

foreach ( $matches as $matchId ) {
    $m = $entityManager->getRepository('Match')->findOneByWsId($matchId);
    if ( $m != null ) {
        continue;
    } else {
        ...
        if ($time > time()) {
            ...
            $stageHome = $entityManager->getRepository('StageTeamStatistics')->findOneBy(array('stage' => $stage->getId(), 'team' => $homeTeam->getId()));
            if ($stageHome == null) {
            ...
            $stageAway = $entityManager->getRepository('StageTeamStatistics')->findOneBy(array('stage' => $stage->getId(), 'team' => $awayTeam->getId()));
            if ($stageAway == null) {
